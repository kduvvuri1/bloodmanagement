...
// Patient routes (from your MongoDB Patient model)
...
// Hospital inventory routes - UPDATED FOR REAL DATA
...
app.get('/api/test', async (req, res) => {
  try {
    ...
    const donorsResult = await pool.query('SELECT COUNT(*) FROM donors');
    ...
    res.json({ 
      ...
      donorsCount: parseInt(donorsResult.rows[0].count),
      hospitalsCount: parseInt(hospitalsResult.rows[0].count),
      patientsCount: parseInt(patientsResult.rows[0].count)
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
